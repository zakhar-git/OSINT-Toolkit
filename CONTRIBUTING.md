...
Contributions are reviewed for accuracy, investigative value, safety, and maintainability. A popular tool is not automatically a suitable tool.
...
5. Add the data to `TOOLS` in `scripts/build_docs.py` under one primary category.
6. Use a reserved domain, IP address, phone number, or synthetic identifier in examples.
7. Run the documentation builder and validation commands.
8. Update README statistics and `CHANGELOG.md` when the catalog count changes.
...
- Put collection frameworks without a clear primary artifact in `Misc`.
...
2. **Source review:** repository, website, license, and capability claims are verified.
3. **Technical review:** installation and example are tested when practical.
4. **Editorial review:** category fit, limitations, navigation, and tone are checked.
5. **Automation:** Markdown, links, TOCs, and generated pages must pass.
6. **Merge:** a maintainer records the change in the changelog when release-relevant.
